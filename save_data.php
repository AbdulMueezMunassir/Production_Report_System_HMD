<?php
// save_data.php - Fixed to save all data including assembly
session_start();
...
$work_hours = (float)($_POST['work_hours'] ?? 11);
...
try {
    ...
    // Prepare save data with proper defaults
    $save_data = [
        'report_date' => $date,
    ...
    $save_data['available_minutes'] = calculateAvailableMinutes($save_data['plan_hours'], $work_hours);
    ...
    $save_data['target_100'] = calculateTarget100($save_data['plan_hours'], $save_data['unit_carder']);
    
    // Calculate day total
    $day_total = 0;
    for ($h = 1; $h <= $work_hours; $h++) {
        $day_total += $save_data["hour_$h"] ?? 0;
    }
    $save_data['day_total'] = $day_total;
    $save_data['acvd_eff'] = calculateAchievedEfficiency(
        calculateEarnedMinutes($day_total, $save_data['unit_smv']),
